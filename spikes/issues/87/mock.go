...
// no API key. It always asks for `bash` once, then reports what it saw.
package main
...
func (m *mockLLM) RoundTrip(req *http.Request) (*http.Response, error) {
	...
	for _, msg := range body.Messages {
		if msg["role"] == "system" {
			m.system, _ = msg["content"].(string)
		}
	}

	// Has a tool result come back yet? If so, finish by quoting it.
	for i := len(body.Messages) - 1; i >= 0; i-- {
		if body.Messages[i]["role"] == "tool" {
			c, _ := body.Messages[i]["content"].(string)
			return resp(map[string]any{"content": "tool said: " + strings.TrimSpace(c)}), nil
		}
	}
	m.calls++
