...
export function partTypeForMime(mimeType) {
...
export function partBytes(part) {
    if (part.type === "text")
...
export function partTokens(bytes) {
...
export function describePart(part) {
    if (part.type === "text")
        return { type: "text", bytes: partBytes(part) };
...
export function summarizePart(part) {
...
export function validatePart(part, maxPartBytes) {
    if (part.type === "text")
        return part;
...
export function isContentPart(v) {
    if (!v || typeof v !== "object")
        return false;
    const t = v.type;
    if (t === "text")
        return typeof v.text === "string";
    if (t === "image" || t === "file" || t === "audio")
...
function isBlobLike(v) {
    return !!v && typeof v === "object" && typeof v.arrayBuffer === "function" && !ArrayBuffer.isView(v);
}
...
function unknownExtension(nameOrPath) {
    const ext = extensionOf(nameOrPath);
    return new ContentPartError(`no mime type for extension "${ext || "(none)"}" — pass an explicit mimeType (mime is never sniffed)`, "unknown-extension");
}
/**
 ...
 */
export async function attach(source, opts = {}) {
    if (typeof source === "string") {
        if (source.startsWith("data:"))
            return fromDataUrl(source, opts);
        if (/^https?:\/\//i.test(source))
            return fromUrl(source, opts);
        return fromPath(source, opts);
    }
    if (isBlobLike(source))
        return fromBlob(source, opts);
    if (!opts.mimeType) {
 ...
 */
export async function fromBlob(blob, opts = {}) {
    const name = typeof blob.name === "string" ? String(blob.name) : undefined;
    const mimeType = opts.mimeType || (blob.type || undefined) || (name ? mediaTypeFor(name)?.mimeType : undefined);
    if (!mimeType)
        throw unknownExtension(name ?? "");
...
export function fromBytes(bytes, mimeType, opts = {}) {
...
export async function fromPath(p, opts = {}) {
    const mimeType = opts.mimeType ?? mediaTypeFor(p)?.mimeType;
    if (!mimeType)
        throw unknownExtension(p);
...
export function fromDataUrl(url, opts = {}) {
    const m = /^data:([^;,]*)(;base64)?,(.*)$/s.exec(url);
    if (!m)
        throw new ContentPartError("malformed data: URL", "source-missing");
...
export function fromUrl(url, opts = {}) {
    const mimeType = opts.mimeType ?? mediaTypeFor(new URL(url).pathname)?.mimeType;
    if (!mimeType)
        throw unknownExtension(new URL(url).pathname);
    return finish(partTypeForMime(mimeType), mimeType, { url }, opts);
}
function finish(type, mimeType, source, opts) {
    const part = { type, mimeType, ...source };
    if (type === "file" && opts.name)
        part.name = opts.name;
    return validatePart(part, opts.maxPartBytes);
}
// --------------------------------------------------------------------------- //
...
const AUDIO_FORMATS = { "audio/mpeg": "mp3", "audio/mp3": "mp3", "audio/wav": "wav", "audio/x-wav": "wav" };
const dataUrl = (mimeType, data) => `data:${mimeType};base64,${data}`;
/**
 ...
 */
export function encodePart(part, style) {
    if (part.type === "text")
...
export function resetUnsupportedWarnings() {
    warnedUnsupported.clear();
}
/** The text a degraded (non-failing) unsupported part leaves behind — never silence. */
export function unsupportedPlaceholder(part) {
    return `[unsupported ${part.type} part (${part.mimeType}, ${partBytes(part)} bytes)]`;
}
...
function warnUnsupportedOnce(style, type, reason, message) {
 ...
 */
export function encodeParts(parts, opts) {
    const mode = opts.onUnsupportedPart ?? (opts.provenance === "attached" ? "error" : "text");
    const out = [];
    for (const raw of parts) {
        ...
        if (opts.maxPartBytes !== undefined && part.type !== "text") {
            ...
            if (bytes > opts.maxPartBytes) {
                if (mode === "error") {
                    throw new ContentPartError(`content part "${part.type}" is ${bytes} decoded bytes, over the maxPartBytes limit of ${opts.maxPartBytes}`, "too-large");
                }
                warnUnsupportedOnce(opts.style, part.type, "too-large", `content part "${part.type}" is ${bytes} decoded bytes, over the maxPartBytes limit of ${opts.maxPartBytes} — sending a text placeholder`);
        ...
        if (mode === "error") {
            throw new ContentPartError(`provider style "${opts.style}" defines no block for a "${part.type}" content part` +
                (part.type !== "text" ? ` (${part.mimeType})` : ""), "unsupported");
        }
 ...
 */
export function inboundPart(block) {
    ...
    if (b.type === "file" && typeof b.file?.file_data === "string") {
        const part = fromDataUrl(String(b.file.file_data));
        part.type = "file";
        if (b.file.filename)
            part.name = String(b.file.filename);
        return part;
    }
