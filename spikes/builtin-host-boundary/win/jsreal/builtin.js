...
function shellCandidates() {
    ...
    if (process.env.COMSPEC)
        out.push([process.env.COMSPEC, "/d", "/s", "/c"]);
    out.push(["cmd.exe", "/d", "/s", "/c"], ["pwsh", "-NoProfile", "-Command"], ["powershell", "-NoProfile", "-Command"], ["bash", "-lc"]);
    return out;
}
/**
...
class BuiltinEnv {
    ...
    constructor(cfg) {
        ...
        this.shell = [];
        this.shellError = new Error(`no shell interpreter found (tried: ${tried.join(", ")}); set builtins.shell, or disable the bash builtin with builtins.tools.bash = false`);
    }
 ...
 */
export function builtinsEnabled(cfg) {
    if (cfg === undefined)
        return true;
 ...
 * are ignored). SPEC §4A.
 */
export function selectBuiltins(cfg) {
    ...
    if (selected.some((t) => t.name === "bash"))
        builtinShell(cfg);
    return selected;
}
const err = (output, metadata) => ({ output, isError: true, metadata });
const ok = (output, metadata) => ({ output, isError: false, metadata });
const IGNORE_DIRS = new Set(["node_modules", ".git"]);
function builtin(name, description, inputSchema, run) {
    return {
        name,
        description,
        inputSchema,
...
function matchGlob(rel, glob) {
    ...
    if (!glob.includes("/"))
        return re.test(path.basename(rel));
    return re.test(rel);
}
...
// individual tools
// ---------------------------------------------------------------------------
function bashTool(env) {
    return builtin("bash", "Run a shell command and return its combined stdout+stderr. Non-zero exit is an error.", {
        type: "object",
        properties: {
    ...
    }, (args, ctx) => new Promise((resolve) => {
        const command = String(args.command ?? "");
        if (!command)
            return resolve(err("bash: command is required"));
        let argv;
        let workdir;
        try {
            argv = env.shellArgv();
            ...
            return resolve(err(`bash: ${e.message}`));
        }
        const timeout = typeof args.timeout === "number" ? args.timeout : 60_000;
        const meta = { shell: env.shellLabel };
        ...
        child.on("close", (code) => {
            if (stopped) {
                ...
                if (!killedTree)
                    killedTree = killJob(false);
                meta.timedOut = stopped === "timeout";
                meta.killedTree = killedTree;
                return finish(err(stopped === "timeout"
                    ? `bash: command timed out after ${timeout}ms\n${out}`
                    : `bash: command cancelled\n${out}`, meta));
            }
            meta.exitCode = code;
            if (code !== 0)
                return finish(err(`${out}\nbash: command exited with code ${code}`, meta));
            finish(ok(out, meta));
        });
    }));
}
function readTool(env) {
    return builtin("read", "Read a file. Recognised media (png/jpg/jpeg/gif/webp/pdf/mp3/wav) comes back as a content part; anything else is read as UTF-8 text, with offset/limit returning that line window.", {
        type: "object",
        properties: {
        ...
        if (media) {
            return {
                ...
                isError: false,
                parts: [{ type: media.type, mimeType: media.mimeType, data: bytes.toString("base64"), ...(media.type === "file" ? { name: path.basename(p) } : {}) }],
            };
        }
        let content;
        try {
...
function writeTool(env) {
    return builtin("write", "Write content to a file (create/overwrite), creating parent directories.", {
        type: "object",
        properties: {
        ...
        const p = String(args.path ?? "");
        if (!p)
            return err("write: path is required");
...
function editTool(env) {
    return builtin("edit", "Exact-string replace in a file. Default replaces a single unique occurrence; replaceAll replaces all.", {
        type: "object",
        properties: {
        ...
        if (count === 0)
            return err(`edit: oldString not found in ${p}`);
        let next;
        if (args.replaceAll === true) {
            ...
            if (count > 1)
                return err(`edit: oldString is not unique in ${p} (${count} occurrences); use replaceAll`);
...
function grepTool(env) {
    return builtin("grep", "Search file contents by regex under a directory. Output is file:line:text matches.", {
        type: "object",
        properties: {
        ...
        for (const file of walkFiles(root)) {
            ...
            for (let i = 0; i < lines.length; i++) {
                if (re.test(lines[i]))
                    hits.push({ rel, line: i + 1, text: lines[i] });
            }
        }
...
function globTool(env) {
    return builtin("glob", "List files matching a glob under a directory. Output is newline-joined relative paths.", {
        type: "object",
        properties: {
...
function webfetchTool() {
    return builtin("webfetch", "HTTP GET a URL and return its body as text, markdown, or html.", {
        type: "object",
        properties: {
        ...
        if (!url)
            return err("webfetch: url is required");
        ...
        try {
            ...
            if (!res.ok)
                return err(`HTTP ${res.status}`, { status: res.status });
 ...
 */
function renderQuestionPrompt(questions) {
    return questions
        .map((q) => {
        const item = q;
...
function questionTool() {
    return builtin("question", "Ask the host one or more questions. Suspends via a kind:\"question\" Request (§10); the host's waitFor resolves it and the answer is returned to the model.", {
        type: "object",
        properties: {
            questions: {
                type: "array",
        ...
        if (ctx?.answer)
            return ok(JSON.stringify(ctx.answer.data ?? {}));
...
function todowriteTool() {
    return builtin("todowrite", "Replace the session todo list. Returns the rendered list.", {
        type: "object",
        properties: {
            todos: {
                type: "array",
...
function applyUpdate(content, body) {
    ...
    for (const hunk of hunks) {
        ...
        if (oldBlock.length > 0) {
            if (!result.includes(oldBlock))
                throw new Error("hunk does not match file contents");
...
function applyPatchTool(env) {
    return builtin("apply_patch", "Apply a patch (Begin/End Patch grammar: Add/Update/Delete File). Atomic — a non-matching hunk aborts with no writes.", {
        type: "object",
        properties: {
        ...
        if (!patchText)
            return err("apply_patch: patchText is required");
        let ops;
        try {
        ...
        try {
            for (const op of ops)
                op.path = env.resolvePath(op.path);
        }
        catch (e) {
            return err(`apply_patch: ${e instanceof Error ? e.message : String(e)}`);
        }
 ...
 */
export function builtinShell(cfg) {
