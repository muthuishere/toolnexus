...
def _kill_job(proc: "Optional[subprocess.Popen[bytes]]", graceful: bool) -> bool:
    ...
    try:
        if sys.platform == "win32":
            ...
            # process in the tree while still being able to take the PARENT
            # down — which reparents the grandchild and leaves it running. The
            # job is terminated at once there; the grace window is POSIX-only.
