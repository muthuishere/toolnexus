...
description: Give your agent a team — delegate subtasks to scoped, isolated sub-agents with one task call, and get back one clean result with tokens rolled up. In all seven languages.
---
...
;; An AgentDef is a plain map; `:team` names the agents `task` can reach.
(def explore
  {:name  "explore"
   :does  "read-only research"
   :tools [lookup]})                        ; least privilege: only these tools
...
(def runtime
  (rt/create-runtime
   {:registry {"explore" explore "coder" coder}
    :llm      {:base-url "https://openrouter.ai/api/v1"
               :style    "openai"
               :model    "openai/gpt-4o-mini"}}))

(def r (rt/run-agent runtime "coder" "fix the failing test"))
(println (:status r) (:text r) (:total-tokens r))
```
</TabItem>
</Tabs>
...
`asTool()` / `as_tool()` / `AsTool()` — and `(rt/agent-tool runtime "explore")` in Clojure —
turns any agent into a plain tool for the classic API's
