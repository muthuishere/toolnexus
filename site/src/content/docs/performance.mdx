...
[Mastra](https://github.com/mastra-ai/mastra) is a batteries-included **TypeScript platform**
(workflows, RAG, evals, a playground, deployers) that toolnexus deliberately is not; it wraps the
Vercel AI SDK and adds its own agent/processor layers, which is why it lands heaviest per call
here. The honest tradeoff — platform breadth vs a portable, low-overhead core that runs
byte-identically in six languages — is written up in full (capability matrix + who-wins-where) in
