...
description: Delegate work to in-process sub-agents — agent() + team + the task tool, with context isolation, usage roll-up, hierarchical budgets, and durable suspension — in all seven languages.
---
...
to another **in-process**: isolated context, one result back, tokens rolled up.
...
`Toolnexus.Agents` (C# and Elixir), `toolnexus.agents.runtime` (Clojure — where an agent is a plain
map in the runtime's `:registry`, not an object).
</Aside>
...
```clojure
(require '[toolnexus.agents.runtime :as rt] '[toolnexus.core :as tn])

;; There is no Agent object here: an AgentDef IS a plain map, and the runtime's
;; :registry is the wiring. :tools scopes the child's toolkit view (scoping is
;; the security model); :team names the only targets of its `task` tool.
(def registry
  {"explore" {:name  "explore"
              :does  "read-only research"
              :tools [lookup]}                   ; this child sees ONLY these tools
   "coder"   {:name   "coder"
              :does   "implements changes"
              :soul-file "AGENTS.md" ; identity file → system prompt
              :team   ["explore"]                ; team = the task tool's only targets
              :budget {:max-tokens 10000}}})

(def runtime
  (rt/create-runtime {:registry registry
                      :llm      {:base-url "https://openrouter.ai/api/v1"
                                 :style    "openai"
                                 :model    "openai/gpt-4o-mini"}}))

(def r (rt/run-agent runtime "coder" "fix the failing test"))
(println (:status r) (:text r) (:total-tokens r))  ; "done", final text, tree-wide tokens
```
</TabItem>
</Tabs>
...
<TabItem label="Clojure">
```clojure
;; agent-tool turns a registry entry into an ordinary Tool
(def tk (tn/build {:tools [(rt/agent-tool runtime "explore")]}))
```
</TabItem>
</Tabs>
...
[the suspension layer](/toolnexus/suspension/), verbatim, no new pending type. The nearest
...
if (r.status === "pending") {
  ...
  // routes to the DEEPEST suspended handle; returns the result of the TOPMOST handle re-run
  const resumed = await r.runtime.resume(answer)
}
```
...
runtime under `r.runtime` · Clojure: `(rt/resume runtime answer)` — you already hold the runtime,
because `run-agent` takes it.)
...
<Aside type="caution" title="Reattachment covers `task` calls — not your own tools">
"Continues from its checkpoint" means the suspended turn is **rewound to its pre-turn snapshot and
replayed**, so **every tool that ran in that turn runs again**. Reattachment by task key is what
keeps a re-run *parent* from spawning duplicate children; a leaf agent's own `git push`, payment or
`POST` has no equivalent protection. The rule to design against:

> **Any tool reachable in a turn that can suspend must be idempotent, or guarded by you.**

This is true on the inline path too — §10 resolves a suspension by re-executing the suspended tool
with `ctx.answer`, so at least one tool is always called twice across a resume. Replaying the
suspended leaf's stored transcript instead of rewinding it is deferred to its own change
(`openspec/changes/fix-consumer-issues-86-93`, D3).
</Aside>
...
status vocabulary is closed and byte-identical in all seven ports:
...
Four rules hold in all seven ports:

1. **Agent beats runtime, replace never merge.** Two transcript rewrites have no defined
   order, so an agent that sets `hooks` runs only its own. The two fields resolve
   independently — override one, inherit the other.
2. **Forwarded verbatim.** The runtime doesn't compose, wrap, reorder or read either value.
   Everything documented for a hand-built client holds identically here.
3. **They can't reach the runtime's own options.** Neither value can change an agent's composed
   soul, its escalation authority, its HTTP seam or the shared conversation store — that's why
   these are two typed fields and not one "configure the client" escape hatch.
4. **Unset changes nothing.** With neither set, a run is byte-identical to one on a runtime that
   never had the fields.

The headline use is **context compaction on a long-lived agent** — a compactor *is* a
`beforeLLM` hook. See [Persona agents → compaction](/toolnexus/persona-agents/#wire-it-on-an-agent-run-per-agent)
for the per-agent snippets in all seven languages.

<Aside type="note" title="Compaction and suspension interact — correctly">
A turn that compacts and *then* suspends is rewound with the rest of the turn: the stored
transcript returns to its full pre-turn state and the resumed replay compacts again. The
compactor is a pure `messages → messages` function, so re-running it is free of surprises — and
the durable-resume guarantee (the parent re-invokes `task` and reattaches by task key) is
untouched.
</Aside>
...
- [Suspension & the human loop](/toolnexus/suspension/) — the machinery escalation rides on.
- [Persona agents](/toolnexus/persona-agents/#staying-under-the-context-window--compaction) — keep a high-tool-volume worker under its context budget with the `compactor` `beforeLLM` hook.
- [A2A — remote agents](/toolnexus/a2a/) — the same axiom across the network; `serve(agent)` is unchanged.
- [SPEC.md §7D](https://github.com/muthuishere/toolnexus/blob/main/SPEC.md#7d-sub-agents--agent-runtime-source-agent) — the full cross-language contract (state machine, gates, cancellation table).
