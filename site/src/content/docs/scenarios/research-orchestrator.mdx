...
already run concurrently in the shipped client loop (`SPEC.md §8`) — and `task` is just a tool.
...
`SPEC.md §7D`). That composed description is the *entire* basis on which the coordinator picks a
...
when full. All three backpressure gates in the runtime are loud by design (`SPEC.md §7D`); none
...
never an exception** (`SPEC.md §7D`). The coordinator's *model* sees the failure in its transcript
alongside the successes and decides what to do: re-prompt the worker, route the question to a
different one, or write the report with a stated gap. Mechanical retry and backoff (§8) already
...
- [Suspension & the human loop](/toolnexus/suspension/) — the §10 primitive escalation and durable resume ride on.
