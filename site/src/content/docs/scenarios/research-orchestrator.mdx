...
in Java/C#/Clojure you reach it by driving the runtime directly rather than through `run()` —
in Clojure that is `(rt/handles runtime)`, which is the same view under a name that does not
shadow `clojure.core/list`. Budget
...
What reattachment does **not** cover is a worker's own tools. "Continues from its checkpoint" means
the suspended turn is rewound and replayed, so every tool the *suspended* agent ran in that turn
runs again — its siblings are safe, its own `git push` is not. Tools reachable in a suspendable
turn must be idempotent, or guarded by you.
...
(def tools (tool/all-tools research))

(defn worker [nm does soul]
  {:name nm :does does :soul soul :tools tools
   :budget {:max-turns 15 :max-tokens 80000}})
...
Finish with a recommendation, the evidence for it, and what would change your mind."
   :team   (mapv :name team)          ; a team is a list of NAMES in the registry
   :budget {:max-turns 25 :max-tokens 600000 :max-concurrent 4 :max-children 8 :max-depth 2}})

(def runtime
  (rt/create-runtime
   {:registry             (into {"coordinator" coordinator} (map (juxt :name identity)) team)
    :llm                  llm
    :max-concurrent-turns 3}))   ; provider-friendly: 4 workers, 3 requests in flight

(def r (rt/run-agent runtime "coordinator"
                     "Should we migrate billing off Stripe? We do ~40k charges/month across US and EU."))

(println (:status r) (:total-tokens r))   ; whole-tree ledger
(println (:text r))                        ; the brief
(doseq [h (rt/handles runtime)]            ; per-handle breakdown, tree order
  (println (:id h) (:state h) (:tokens h)))
```
</TabItem>
</Tabs>
