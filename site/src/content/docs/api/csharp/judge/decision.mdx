...
**A response that omits `calibrated`, or sends `null`, decodes as `true`** — only the literal
`false` is false. The systemone wire reports calibration by being itself, and a backend that is
not calibrated says so explicitly, so a missing field is not a missing guarantee.
