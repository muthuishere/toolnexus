...
C# drops the `Mcp` from the method name because it already lives on `McpSource`. Java does the same for the same
reason — [`McpSource.parseConfig`](/api/java/mcp/parse-config/). The remaining ports spell it as a
free function: `parseMcpConfig` / `parse_mcp_config` / `ParseMcpConfig` / `parse-config`.
</Aside>
