...
func main() {
	...
	// The out-of-band Answer resumes the DEEPEST suspended handle, and Resume
	// hands back the result of the topmost handle the cascade re-ran.
	final, err := rt.Resume(tn.Answer{ID: r.Pending.ID, Ok: true})
	if err != nil {
		log.Fatal(err)
	}
	if final.Status != "done" || rt.StateOf(h) != agents.StateIdle {
		log.Fatalf("expected done+idle after resume, got status=%q state=%s", final.Status, rt.StateOf(h))
	}
