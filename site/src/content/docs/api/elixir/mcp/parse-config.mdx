...
| C# | `McpSource.ParseConfig` | [→](/api/csharp/mcp/parse-config/) |

<Aside type="note" title="Error convention differs">
Elixir raises (from `Jason.decode!/1` or `File.read!/1`), as do JavaScript, Python and C#. Go
returns `(McpConfig, error)`. Java exposes no public parser at all.
</Aside>
