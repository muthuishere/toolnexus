...
<Aside type="note" title="Same contract, each language's error convention">
JavaScript catches a thrown `Error`; Go's tool function returns an `error` that is folded into
`isError`; Java and C# catch exceptions. In all six, a failing native tool reports `isError` — it
never blows up the loop.
</Aside>
