...
{:name "read" :description "Read a file. A known media file (png/jpg/…) comes back as a content part; …"
...
| `read` | Reads a file. A known media file (`png jpg jpeg gif webp pdf mp3 wav`) comes back as a [content part](/api/clojure/types/content-part/) under `:parts`; anything else is read as UTF-8 text. | Optional 1-based `offset` and `limit` on the text path. Bytes that are not decodable UTF-8 give an `isError` result rather than mojibake. |
