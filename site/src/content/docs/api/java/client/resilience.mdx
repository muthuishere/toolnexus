...
public Options retryableStatuses(List<Integer> v)       // extra retryable statuses, ADDED to the defaults
...
attempt (`429`, `500`, `502`, `503`, `504`, `529` and network errors are `retryable=true` by
default) and returns a `Tier` — `RETRY` (bounded
...
`onError` when the default "retry `429`/`500`/`502`/`503`/`504`/`529`/network, fail everything
else" isn't right for your host — e.g. a budget-conscious caller that never wants to retry a rate
limit, or one that wants to retry a normally-terminal status because the upstream is flaky in a
nonstandard way. That default set is an exhaustive enumeration, not "429 plus any 5xx": a `501`, a
`507` or a Cloudflare `520` is terminal unless you say otherwise. `retryableStatuses` when the
upstream emits a transient status outside the set — the motivating case is a Cloudflare-fronted
origin answering `520`–`527`. `timeoutMs` whenever a run must not hang past a caller-facing SLA.
`CancelToken` whenever something OTHER than the run itself decides to abort it — a user closing a
tab, a supervisor killing a stuck job.

```java
LlmClient client = LlmClient.create(new LlmClient.Options()
        .model(model)
        .apiKey(apiKey)
        .retryableStatuses(List.of(520, 521, 522, 523, 524, 525, 526, 527)));
```

`retryableStatuses` is **additive to the defaults, never a replacement** — it can only make more
statuses retryable, never fewer, so setting it cannot accidentally drop `429` and lose
...
`429`/`500`/`502`/`503`/`504`/`529`/network (plus anything in `retryableStatuses`). Most callers
never need `onError` at all — set `retries`/`retryBaseMs` to tune
...
public class Example {
    public static void main(String[] args) throws Exception {
        ...
        try (Toolkit tk = Toolkit.create(new Toolkit.Options())) {
            // No onError set — the default classifier treats 429/500/502/503/504/529/network as RETRY, everything
...
| `Options.retryableStatuses(v)` | `List<Integer>` | Extra HTTP statuses to classify as retryable, **added** to the default `429`/`500`/`502`/`503`/`504`/`529`. Widens only, never shrinks; `onError` still has the final say. `null` ⇒ defaults only. |
...
`Classifier` carries the same knob under the same name: `Classifier.Options.retryBaseMs` (default `500`, `base * 2^attempt`, no jitter, `Retry-After` still wins). See [`Classifier`](/api/java/judge/classifier/).
