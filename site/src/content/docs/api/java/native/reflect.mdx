...
<Aside type="note" title="Elixir has no reflection-based tool builder">
Use `Toolnexus.Native.define_tool/1` with an explicit `input_schema` instead — the same outcome,
written by hand rather than inferred.
</Aside>
