...
- [ ] 2.2 Python: `request_params` + `body_transform` + injectable `httpx` client; omit empty tools. Tests + green.
...
- [ ] 2.4 Java: `requestParams` + `bodyTransform` + injectable `HttpClient`; omit empty tools. Tests + green.
- [ ] 2.5 C#: `RequestParams` + `BodyTransform` + injectable `HttpClient`/handler; omit empty tools. Tests + green.
...
- [ ] 3.2 Python: cancellation plumbing; `tools` allowlist; `list_mcp_tools`. Tests + green.
...
- [ ] 3.4 Java: cancellation plumbing; `tools` allowlist; `listMcpTools`. Tests + green.
- [ ] 3.5 C#: `CancellationToken` plumbing; `tools` allowlist; `ListMcpTools`. Tests + green.

## 4. Cluster C — conversation-store accessor (gap 4) · §8

- [x] 4.1 JS `client.conversationStore` getter (green); [x] 4.3 Go `(*Client).ConversationStore()` (green); [ ] 4.2 Python; [ ] 4.4 Java; [ ] 4.5 C#. Identity + default + shared-reuse tests.

## 5. Verify & release

- [ ] 5.1 All five suites green; conformance parity (empty-tools omission + allowlist fixture identical across ports).
- [ ] 5.2 `openspec validate implement-rag-go-consumer-needs`.
- [ ] 5.3 Bump all four manifests to 0.9.0; PR; CI green; merge; cut release v0.9.0; archive.
