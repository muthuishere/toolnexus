...
- [ ] 0.1 Confirm with `add-agent-pipeline` that its `retry` combinator delegates to this
      `onError` seam (design D4) — do not apply until settled

## 1. Spec
- [ ] 1.1 `SPEC.md §8` Resilience — add the `onError` classifier + two tiers (retry/fail);
...
- [ ] 2.2 One branch in `fetchWithRetry`: resolved classifier → retry (bounded) | fail
- [ ] 2.3 Resilience matrix tests (400/401/402/429/500/network + absent-onError parity),
...
- [ ] 3.5 Elixir — `on_error`, retry-helper branch, matrix
